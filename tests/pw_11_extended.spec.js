...
test('Визуальная регрессия главной страницы с помощью скриншота', async ({ page }) => {
    await page.goto(url);

    await expect(page).toHaveScreenshot(
        'homePage.png',
        {
            fullPage: true, //снимок полной страницы
            mask: [
                page.locator('.tag-pill'), //маскируем тэги
                page.locator('.date'), //маскируем даты
                page.locator('.counter'), //маскируем тэги
            ]
        }
    );
...
test.only('Визуальная регрессия главной страницы с помощью снапшота', async ({ page }) => {
    await page.goto(url);

    const $body = page.locator('#root'); //снапшот (древо - dom) всей страницы
